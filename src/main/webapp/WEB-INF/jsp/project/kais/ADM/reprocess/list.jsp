...
<script src="https://ssl.daumcdn.net/dmaps/map_js_init/postcode.v2.js"></script>
		...
		<div class="row">
							...
							<tbody>
								<tr>
									<td style='width:100%;text-align:right;'>
										<button type="button" class="btn btn-info btn-icon icon-left" onclick="javascript:fn_refresh(1);" authUri='/ADM_REP_S1000A/'>
											검색
			...
			<div class="col-sm-12" style="padding:0px;margin:0px;">
				<div style="width:100%;right-margin:5px;text-align:right;padding-bottom:5px;">
					<span id='TCNT' style='font-weight:bold;color:#ff6600;'></span>건이 조회되었습니다.
				</div>
				<div style="width:100%;right-margin:5px;text-align:right;padding-bottom:5px;">				
					<p style="width:100%;text-align:right;margin-top:10px;">
						<button type="button" class="btn btn-orange btn-icon" onclick="javascript:fn_reprocess();">
							재처리 <i class="entypo-check"></i>
						</button>
					</p>
				</div>
...
function fn_set_table(){
	...
	table	= $('#TBKW_SCDL').DataTable( {
		...
		,columns: [
			...
			,{ data:"SCDL_RTCD",    name: "SCDL_RTCD",    title: "스크래핑 결과코드", align: "left",  width: 100, orderable: false}
			,{ data:"SCDL_CNT",     name: "SCDL_CNT",     title: "스크래핑 처리횟수", align: "right", width: 100, orderable: false}
			,{ data:"SCDL_RTMSG",   name: "SCDL_RTMSG",   title: "처리메시지",      align: "left",  width: 100, orderable: false}		
			,{ data:"SCDL_NRTCD",   name: "SCDL_NRTCD",   title: "정보등록 결과코드", align: "left",  width: 100, orderable: false}
			,{ data:"SCDL_CNT2",    name: "SCDL_CNT2",    title: "정보등록 처리횟수", align: "right", width: 100, orderable: false}	
			,{ data:"SCDL_NRLCD",   name: "SCDL_NRLCD",   title: "로그등록 결과코드", align: "left",  width: 100, orderable: false}
			,{ data:"SCDL_CNT3",    name: "SCDL_CNT3",    title: "로그등록 처리횟수", align: "right", width: 100, orderable: false}
...
function fn_refresh(no){
	...
	$.ajax({
		...
		,data: {
			YYMM:$('#YYMM').val()
			,SCDL_RDT_FR:$("#SCDL_RDT_FR").val().replaceAll('-','')
			,SCDL_RDT_TO:$("#SCDL_RDT_TO").val().replaceAll('-','')
