...
import com.kwic.sql.SqlDebuger;
...
public class MappedStatement {
  ...
  public int executeUpdate(StatementScope statementScope, Transaction trans, Object parameterObject)
    ...
    try {
      ...
      if(DaoSupport.debugQuery()){
          SqlDebuger.debug("--------------- "+id+"----------------");
          SqlDebuger.debug(debugQuery(sqlString,parameters));
          SqlDebuger.debug("--------------------------------------");
      }
...
public void addResultMap(ResultMap resultMap) {
  ...
  public String debugQuery(String query,Object[] arr){
	  ...
	  for(int i=0;i<arr.length;i++){
		  if(sb.indexOf("?")<0)
			  break;
		  sb.replace(sb.indexOf("?"), sb.indexOf("?")+1, "'"+String.valueOf(arr[i])+"'");
	  }
	  return sb.toString();
  }
}
