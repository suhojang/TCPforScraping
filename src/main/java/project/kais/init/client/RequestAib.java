 ...
 * 3. DB에서 대상건 조회
 */
public class RequestAib extends InitServiceImpl {	

	private static Logger log = LoggerFactory.getLogger(RequestAib.class);
	 ...
	 * 참고 : init-service.xml
	 */
	private static List<AibClientThread> threadtList = new Vector<AibClientThread>();
	...
	public void execute() throws Exception {
		...
		initParam.put("SCDL_STS",    KaisState.AIB_TGT_A0); //처리상태 - AIB요청 대상
		initParam.put("BF_SCDL_STS", KaisState.AIB_ASN_AA); //처리상태 - AIB요청 대상 지정
		 ...
		 */
		AibClientThread thread = null;
		for (int i = 0; i < getServiceParamInt("pool-size"); i++) {
			thread = new AibClientThread();
		...
		while (!stop) {
			...
			try {
				 ...
				 */
				try {
					param.put("SCDL_STS", KaisState.AIB_TGT_A0);
					param.put("RETRY_CNT", getServiceParamString("RETRY_CNT"));
					targetList = service.SELECT_REQUEST_AIB_LIST(param);
					//log.debug(String.format("[DB-SELECT_REQUEST_AIB_LIST] AIB 목록 조회. param=%s, count=%d", param, targetList.size()));
				} catch (Exception e) {
				...
				for (int i = 0; targetList != null && i < targetList.size(); i++) {
					...
					// 대상건 상태를 진행중(AA)으로
					param.put("SCDL_STS", KaisState.AIB_ASN_AA);
					service.UPDATE_REQUEST_AIB(param);
					log.debug(String.format("[DB-UPDATE_REQUEST_AIB] AIB 전송전 상태 변경. SCDL_SEQ=%s, SCDL_STS=%s, SCDL_CNT=%s", param.get("SCDL_SEQ"), param.get("SCDL_STS"), param.get("SCDL_CNT") ));
